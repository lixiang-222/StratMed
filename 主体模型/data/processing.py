...
def get_ddi_matrix(records, med_voc, ddi_file):
    ...
    for patient in records:
        for adm in patient:
            ...
            for diag in adm[0]:
                nodes.append(diag)
            for pro in adm[1]:
                nodes.append(pro + voc_size[0])
            for med in adm[2]:
                nodes.append(med + voc_size[0] + voc_size[1])
    ...
    # 计算直方图
    hist, bins = np.histogram(ehr_adj, bins=[0, 10, 100, 300, 500, 1000, 5000, 10000, 20000]) #分了8个桶
    # hist, bins = np.histogram(ehr_adj,
    #                           bins=[0, 10, 20, 30, 40, 50, 100, 200, 300, 400, 500, 1000, 2000, 3000, 4000, 5000, 6000,
    #                                 7000, 8000, 9000, 10000, 20000]) # 分了21个桶
    # 获取每个元素所属的桶
    buckets = np.digitize(ehr_adj, bins)

    dill.dump(buckets, open('./output/ehr_adj_final.pkl', 'wb'))
