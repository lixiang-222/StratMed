...
class DGN_SW(nn.Module):
    def __init__(self, in_feartures, out_features, edge_types):
        super(DGN_SW, self).__init__()
        self.edge_types = edge_types

        self.edge_embedding = nn.Embedding(edge_types + 1, 1)

        self.lin = nn.Linear(in_feartures, out_features)

        self.init_weight()

    def forward(self, x, edges, edge_type):
        weights = self.edge_embedding(edge_type)

        result = torch.zeros_like(x)
        for source, target, weight in zip(edges[0], edges[1], weights):
            result[target] += weight * x[source]
    ...
    def init_weight(self):
        ...
        for i in range(self.edge_types + 1):
            self.edge_embedding.weight.data[i].copy_(weight * i)


class DGN_MF(nn.Module):
    def __init__(self, in_feartures, out_features, edge_types):
        super(DGN_MF, self).__init__()
        self.edge_types = edge_types
        self.emb_dim = in_feartures

        self.edge_embedding = nn.Embedding(edge_types + 1, in_feartures)
    ...
    def forward(self, x, edges, edge_type):
        ...
        result = torch.zeros_like(x)
        for source, target, weight in zip(edges[0], edges[1], weights):
            result[target] += torch.mul(weight, x[source])
...
class GraphNet(nn.Module):
    def __init__(self, voc_size, emb_dim, pretrained_embedding, ehr_adj, device):
        ...
        # 三个分着的同构图
        # self.conv_iso = nn.ModuleList([GCNConv(emb_dim, emb_dim) for _ in range(3)])
        self.conv_med_med = DGN_MF(emb_dim, emb_dim, 8)

        # 异构图训练
        # self.conv_hetero_diag_pro = GCNConv(emb_dim,emb_dim)
        self.conv_hetero_diag_med = GCNConv(emb_dim, emb_dim)
        self.conv_hetero_pro_med = GCNConv(emb_dim, emb_dim)
    ...
    def forward(self, adm):
        ...
        if len(new_adm[2]) == 0:
            ...
            # 开始训练
            # x = self.conv_iso[0](x, edge_index, edge_weight)
            x = self.conv_med_med(x, edge_index, edge_weight)
        x = F.relu(x)  # 里面是按照诊断，程序，药物，顺序排列的节点的结果(n*64)
        ...
        #     edge_weight = self.edges2weight(edges)
        #     x1 = self.conv_hetero_diag_pro(x, edge_index, edge_weight)  # n*64
        # else:
        #     x1 = torch.zeros([len(nodes), self.emb_dim], dtype=torch.float).to(self.device)

        # 构建诊断-药物的异构图
        edges = self.build_heterogeneous_edges(new_adm, 2, 0)
        if len(edges) != 0:
            edge_index = self.node2complete_graph2coo(nodes, edges)  # 利用图节点和边变成pyg需要的coo模式
            edge_weight = 0.1 * self.edges2weight(edges)
            x2 = self.conv_hetero_diag_med(x, edge_index, edge_weight)  # n*64
        else:
        ...
        if len(edges) != 0:
            edge_index = self.node2complete_graph2coo(nodes, edges)  # 利用图节点和边变成pyg需要的coo模式
            edge_weight = 0.1 * self.edges2weight(edges)
            x3 = self.conv_hetero_pro_med(x, edge_index, edge_weight)  # n*64
        else:
    ...
    def build_heterogeneous_edges(self, new_adm, type_a, type_b):
        ...
        # for node1 in new_adm[type_b]:
        #     for node2 in new_adm[type_a]:
        #         edges.append([node1, node2])
    ...
    def residual_output(self, x, adm):
        ...
        # # 对比模型其他细节的测试
        # i1 = embedding_pretrained[0]
        # i2 = embedding_pretrained[1]
        # i3 = embedding_pretrained[2]
...
class AdvancedModel(nn.Module):
    def __init__(
        ...
        self.graph_embeddings = GraphNet(vocab_size, emb_dim, pretrained_embeddings, ehr_adj, device)
