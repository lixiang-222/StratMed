...
def get_ddi_matrix(records, med_voc, ddi_file):
    ...
    #
    # def stratification(data, grad, layers):
    #     percent = grad
    #     sorted_values = np.sort(data.flatten())
    #     # 计算位置
    #     index_list = [0]
    #     for i in range(layers - 1):
    #         percentile_index = int(len(sorted_values) * (1 - percent))
    #         index_list.append(percentile_index)
    #         percent *= grad
    #     # 获取阈值
    #     threshold = sorted_values[index_list]
    #     # threshold = np.insert(threshold, 1, 1)
    #     # 分层
    #     layers = np.digitize(data, threshold)
    #     return layers
    #
    # # 梯度与层数个数
    # grad = 0.5
    # med_med_bucket = stratification(ehr_adj_med_med, grad, 9)  # 分9层
    # med_diag_bucket = stratification(ehr_adj_med_diag, grad, 13)  # 分13层
    # med_proc_bucket = stratification(ehr_adj_med_proc, grad, 13)  # 分13层
    #
